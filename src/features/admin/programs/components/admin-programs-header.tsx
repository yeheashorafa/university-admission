...
type AdminProgramsHeaderProps = {
  onAddProgram: () => void;
};

export function AdminProgramsHeader({ onAddProgram }: AdminProgramsHeaderProps) {
    ...
    <header className="flex flex-col justify-between gap-4 border-b border-border pb-6 lg:flex-row lg:items-end">
      <div>
        ...
        <h1 className="text-3xl font-bold text-primary md:text-4xl">
          {t("programs.managementTitle")}
        </h1>

        <p className="mt-3 max-w-3xl leading-7 text-muted-foreground">
          {t("programs.managementDescription")}
        </p>
      </div>
